...
function [handle,Zi,grid,Xi,Yi] = topoplot_dc(Values,loc_file,varargin)
...
GRID_SCALE = 2*67;        % plot map on a 67X67 grid
CIRCGRID   = 1*201;       % number of angles to use in drawing circles
...
%       [Xi,Yi,ZiC] = griddata(inty,intx,double(intContourVals),yi',xi,'v4'); % interpolate data
      ...
      [Xi,Yi,Zi] = griddata(inty,intx,double(intValues),yi',xi,'nearest'); % interpolate data
      [Xi,Yi,ZiC] = griddata(inty,intx,double(intContourVals),yi',xi,'nearest'); % interpolate data
  catch,
  ...
  % mask out data not near an electrode
  mask_elec = zeros(size(Xi));
  for i=1:length(intx)
      mask_elec = mask_elec + (sqrt((Xi-inty(i)).^2 + (Yi-intx(i)).^2) <= 0.04);
      %mask_elec = mask_elec + (sqrt((Xi-inty(i)).^2 + (Yi-intx(i)).^2) <= 0.02);% check if grid point is close enough to electrode i
  end
  ii = find(mask_elec == 0); % all grid points not near an electrode
  Zi(ii) = 0;
